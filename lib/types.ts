...
export interface Version {
  id: string;
  number: string; // Just "0", "1", "2" - no branch notation in display
  content: string;
  prompt: string | null;
  ...
  isArchived?: boolean; // Archive/cross out versions (for alternatives)
  isGenerating?: boolean; // Currently being generated by AI
  projectConfig?: ProjectConfig; // Only for v0 - project configuration
  // V2: New fields for staging system
...
export type VersionState = 
  | 'ai-created'      // Just created by AI, pending review (STAGED)
  | 'manually-edited' // User made changes after AI
  | 'reviewed'        // Accepted/finalized (FINAL)
  | 'archived';       // Old, not in main history

// V2: Change author for track changes
export interface ChangeAuthor {
  id: string;
  name: string;
  color: string; // purple for AI, orange for user, green for others
  type: 'ai' | 'user';
}

// V2: Individual change for markup
...
export interface EditorState {
  ...
  // V2: New fields
  documentMode: DocumentMode; // Current editing mode
  workingContent: string; // Current unsaved content
  stagedChanges: TrackedChange[]; // Changes pending review in suggesting mode
}
